...
const getCategoryStyle = (category) =>
  CATEGORY_STYLES[category] ?? { bg: "bg-gray-50 dark:bg-gray-500/10", text: "text-gray-700 dark:text-gray-300", dot: "bg-gray-400" };

const InterviewPrepPanel = ({ resumeId }) => {
  const dispatch = useDispatch();
  const { status, historyStatus, error, quotaExhausted, questions, history } =
  ...
  const handleGenerate = () => {
    dispatch(generateInterviewQuestions({ resumeId, targetRole, jobDescription }))
      ...
      .then(() => {
        ...
        dispatch(loadInterviewHistory({ resumeId }));
      })
      .catch((payload) => {
        if (!payload?.quotaExhausted) toast.error(payload?.message || "Generation failed");
      });
  };
  ...
  const QuestionList = ({ qs }) => (
    <div className="space-y-2">
      {qs.map((q, i) => {
        ...
        return (
          <div key={i} className="rounded-lg border border-line overflow-hidden transition-shadow hover:shadow-sm">
            <button
              type="button"
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <span className={`mb-1.5 inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-xs font-medium ${style.bg} ${style.text}`}>
    ...
    <div className="space-y-5">
      ...
      <div className="flex gap-1 border-b border-line">
        {[
          ...
          { id: "history",  label: `History${history.length > 0 ? ` (${history.length})` : ""}`, icon: History },
        ].map((tab) => (
          <button
            key={tab.id}
      ...
      {activeTab === "generate" && (
        <>
          ...
          <div className="space-y-3">
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-ink">
                Job Description Snippet <span className="text-muted font-normal">(optional, ≤ 500 chars)</span>
              </label>
              <textarea
          ...
          {quotaExhausted && (
            <div className="flex items-start gap-3 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-300">
              ...
              <div>
                ...
                <p className="text-amber-700 dark:text-amber-400">Free plan allows 3 question sets per day. Upgrade to premium for unlimited access.</p>
              </div>
            </div>
          )}
          ...
          {!hasResults && (
            ...
            >
              ...
              {isLoading ? "Preparing questions…" : "Generate practice questions"}
            </button>
          )}

          {/* Error */}
          {error && status === "failed" && !quotaExhausted && (
            <p role="alert" className="rounded-lg border border-red-200 bg-red-50 px-4 py-2 text-sm text-red-600 dark:border-red-500/30 dark:bg-red-500/10 dark:text-red-400">
              {error}
            </p>
          )}
          ...
          {hasResults && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <p className="text-sm font-medium text-ink">{questions.length} questions generated</p>
                <div className="flex items-center gap-2">
                  ...
                  >
                    <ClipboardList className="size-3.5" /> Copy All
                  </button>
                  <button
                    type="button"
        ...
        <div className="space-y-4">
          {historyStatus === "loading" ? (
            ...
            <div className="rounded-lg border border-dashed border-line py-10 text-center text-sm text-muted">
              No saved question sets yet. Generate some questions first.
            </div>
          ) : (
            history.map((set) => (
              <div key={set.setId} className="rounded-lg border border-line overflow-hidden">
                <div className="flex items-center justify-between bg-canvas px-4 py-3">
                  <div>
                    <p className="text-sm font-medium text-ink">{set.targetRole || "General Questions"}</p>
                    <p className="text-xs text-muted">{new Date(set.createdAt).toLocaleDateString()} · {set.questions.length} questions</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => {
                      dispatch(loadSavedSet({ questions: set.questions }));
                      setActiveTab("generate");
                      setExpandedIndex(0);
                    }}
