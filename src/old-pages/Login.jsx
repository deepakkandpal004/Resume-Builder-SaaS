...
const Login = () => {
  ...
  const syncUserWithBackend = React.useCallback(async (firebaseUser) => {
    console.log("[Auth/Sync] Requesting backend sync for Firebase user:", {
    ...
    console.log("[Auth/Sync] Backend sync success:", data.user);
  ...
  const sendLoginNotification = React.useCallback(async (firebaseUser, via) => {
    try {
      console.log("[Auth/Notify] Sending login notification via:", via);
      ...
      console.log("[Auth/Notify] Login notification info:", err.message);
    }
  }, []);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    console.log(`[Auth/${state}] Submitting credentials:`, { email: formData.email, name: formData.name });
    try {
      let userCredential;
      if (state === "register") {
        try {
          userCredential = await createUserWithEmailAndPassword(auth, formData.email, formData.password);
          console.log("[Auth/Register] Firebase user created successfully:", userCredential.user.uid);
          if (formData.name && userCredential.user) {
            try {
              const { updateProfile } = await import("firebase/auth");
              await updateProfile(userCredential.user, { displayName: formData.name });
              console.log("[Auth/Register] Updated displayName:", formData.name);
            } catch (pErr) {
              console.warn("[Auth/Register] Could not update profile name:", pErr);
            }
          }
        } catch (createError) {
          console.error("[Auth/Register] Firebase create user failed:", createError);
        ...
        console.log("[Auth/Login] Firebase login success:", userCredential.user.uid);
      }
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      console.log("[Auth/Google] Firebase Popup Success:", {
        uid: result.user.uid,
      ...
      console.error("[Auth/Google] Error during Google Sign-in:", error);
        ...
        {forgotMode ? (
          ...
          >
            ...
            <div className="mb-8 text-center">
              <Link href="/" className="inline-flex items-center gap-1.5 text-sm text-muted transition hover:text-emerald-600">
