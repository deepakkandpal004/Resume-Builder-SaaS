...
const ClassicTemplate = ({ data, accentColor, styleOptions = {} }) => {
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "0.75em" }}>
          {data.project.map((proj, i) => (
            <div key={i} style={{ paddingLeft: "0.9em", borderLeft: `3px solid ${accent}` }}>
              ...
              {proj.type && (
                <div style={{ fontSize: "0.8em", color: accent, marginTop: "0.15em", ...cStyle }}>{proj.type}</div>
              )}
              {proj.description && (
              ...
              {(proj.techStack?.length > 0) && (
                <div style={{ display: "flex", flexWrap: "wrap", gap: "0.35em", marginTop: "0.35em" }}>
                  {proj.techStack.map((tech, ti) => (
                    <span key={ti} style={{ fontSize: "0.72em", padding: "0.1em 0.5em", borderRadius: "3px", background: "#f3f4f6", color: "#374151", border: "1px solid #e5e7eb" }}>
                      {tech}
                    </span>
                  ))}
                </div>
  ...
  const renderCertifications = () =>
    data.certifications?.length > 0 ? (
      <section style={{ marginBottom: "1.5em" }}>
        <h2 style={{ ...sectionHeadingBase, ...hStyle }}>{heading("certifications")}</h2>
          ...
          {data.certifications.map((cert, i) => (
            <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
              <div>
                ...
                {cert.issuer && (
                  <span style={{ color: "#6b7280", marginLeft: "0.4em" }}>· {cert.issuer}</span>
                )}
                {cert.credential_url && (
                  <a
  ...
  const renderLanguages = () =>
    data.languages?.length > 0 ? (
      <section style={{ marginBottom: "1.5em" }}>
        <h2 style={{ ...sectionHeadingBase, ...hStyle }}>{heading("languages")}</h2>
        <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5em 2em" }}>
          {data.languages.map((lang, i) => (
            <span key={i} style={{ color: "#374151", ...cStyle }}>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: "0.25em 1em", fontSize: "0.85em", color: "#6b7280", ...cStyle }}>
